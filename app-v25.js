...
const OMDB_GENRE_DETAIL_LIMIT = 60;
...
async function searchOmdbByGenre(genre) {
  ...
  const resultGroups = await Promise.all(seeds.map((seed) => searchOmdbSummaries(seed, OMDB_GENRE_SEARCH_PAGES)));
  const candidates = uniqueOmdbMovies(resultGroups.flat()).slice(0, OMDB_GENRE_DETAIL_LIMIT);
  const settledDetails = await Promise.allSettled(candidates.map((movie) => fetchOmdbDetails(movie.imdbID)));
